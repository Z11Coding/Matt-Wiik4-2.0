...
class ChartingState extends MusicBeatState
{
	...
	**/
	...
	override function create()
	{
		...
		updateGrid();
		difficulty = CoolUtil.difficultyString().toLowerCase();
		super.create();
	}
	...
	function addSongUI():Void
	{
		...
		var stepperSpeed:FlxUINumericStepper = new FlxUINumericStepper(10, stepperBPM.y + 35, 0.1, 1, -10, 1000, 1);
		...
		var player4DropDown = new FlxUIDropDownMenuCustom(player2DropDown.x + 200, player3DropDown.y + 40,
			FlxUIDropDownMenuCustom.makeStrIdLabelArray(characters, true), function(character:String)
		{
		...
		var difficultyDropDown = new FlxUIDropDownMenuCustom(stageDropDown.x, player3DropDown.y,
			FlxUIDropDownMenuCustom.makeStrIdLabelArray(CoolUtil.defaultDifficulties, true), function(difficulty:String)
		{
			PlayState.storyDifficulty = Std.parseInt(difficulty);
			try
			{
				PlayState.SONG = Song.loadFromJson(_song.song.toLowerCase() + '-' + CoolUtil.defaultDifficulties[PlayState.storyDifficulty], _song.song.toLowerCase());
				MusicBeatState.resetState();
			}
			catch (e:Any)
			{
		...
		tab_group_song.add(new FlxText(player4DropDown.x, player4DropDown.y - 15, 0, 'Opponent 2:'));
	...
	var check_bothSection:FlxUICheckBox;
	...
	function addSectionUI():Void
	{
		...
		check_exSection = new FlxUICheckBox(130, 60, null, null, "EX section", 100);
		check_exSection.name = 'check_ex';
		check_exSection.checked = _song.notes[curSec].exSection;
		// _song.needsVoices = check_mustHit.checked;

		check_bothSection = new FlxUICheckBox(check_gfSection.x + 130, 30, null, null, "Both section", 100);
		check_bothSection.name = 'check_both';
		check_bothSection.checked = _song.notes[curSec].bothSection;
		// _song.needsVoices = check_mustHit.checked;
	...
	override function getEvent(id:String, sender:Dynamic, data:Dynamic, ?params:Array<Dynamic>)
	{
		if (id == FlxUICheckBox.CLICK_EVENT)
		{
			...
			switch (label)
			{
				...
				case 'Both section':
					_song.notes[curSec].bothSection = check.checked;
	...
	override function update(elapsed:Float)
	{
		...
		strumLine.y = getYfromStrum((Conductor.songPosition - sectionStartTime()) / zoomList[curZoom] % (Conductor.stepCrochet * _song.notes[curSec].lengthInSteps));
	...
	function updateHeads():Void
	{
		...
		var healthIconP4:String = loadHealthIconFromCharacter(_song.player4);
		...
		if (_song.notes[curSec].mustHitSection)
		{
			...
			if (_song.notes[curSec].exSection)
				leftIcon.changeIcon(healthIconP4);
		}
		else
		{
	...
	function setupNoteData(i:Array<Dynamic>, isNextSection:Bool):Note
	{
		...
		note.y = (GRID_SIZE * (isNextSection ? 16 : 0)) * zoomList[curZoom]
			+
			Math.floor(getYfromStrum((daStrumTime - sectionStartTime(isNextSection ? 1 : 0)) % (Conductor.stepCrochet * _song.notes[curSec].lengthInSteps),
				false));
		return note;
	}
